...
class SASFlowChart(object):
    ...
    def saveFig(self, path):
        try:
            ...
            self.nodeColors = list(set([value for key, value in dict(
                nx.get_node_attributes(self.G, 'lib')).items()]))
            cmap = plt.get_cmap('Pastel2')
            colors = cmap(np.linspace(0, 1, len(self.nodeColors)))
            self.nodeColors = dict(zip(self.nodeColors, colors))

            colorList = [self.nodeColors[value] for key, value in dict(
                nx.get_node_attributes(self.G, 'lib')).items()]
            ...
            nx.draw_networkx_labels(
                self.G, pos=self.pos, labels=self.nodeLabels, font_size=8)

            self.edge_labels = nx.get_edge_attributes(self.G, 'label')
            self.edge_labels = nx.draw_networkx_edge_labels(
                self.G, self.pos, edge_labels=self.edge_labels, font_size=8, alpha=0.4)
            ...
            legendHandles = [
                mpatches.Patch(
                    color=value,
                    label=key) for key,
                value in self.nodeColors.items()]
    ...
    def _hierarchy_pos(self, G, root, width=1., vert_gap=0.2,
