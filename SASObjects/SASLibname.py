...
class SASLibname(SASBaseObject):
    ...
    def __init__(self,rawStr):
        ...
        self.name = re.findall(r"libname (.{0,8}) ['\"\(][^'\"\(\)]*?['\"\)]\s*;",rawStr,flags=self.regexFlags)[0]
        self._path = re.findall(r"libname .{0,8} ['\"\(]([^'\"\(\)]*?)['\"\)]\s*;",rawStr,flags=self.regexFlags)[0]
...
class SASSQLLibname(SASBaseObject):
    ...
    def __str__(self):
        return '{}: {}.{} // {}'.format(self.name,self.database,self.schema,self.server)
